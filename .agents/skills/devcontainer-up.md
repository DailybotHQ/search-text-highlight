...
description: Spin up the local Docker dev environment and verify it against this repo's npm scripts
type: skill
---

# Skill: `/devcontainer-up`

Bring up the project's Docker devcontainer, log into the AI CLIs, run the smoke checks, and (optionally) repair the inherited helper aliases that don't match this repo's npm scripts.

## When to use

- First-time setup on a new machine
- Local Node toolchain is misbehaving and you want CI parity
...
- **Whether to touch `docker/custom_commands.sh`** — helpers are already wired to this repo's npm scripts; change only if you are extending them
...
The first build pulls Node 24 (slim trixie), installs Chromium, GitHub CLI, and the three AI CLIs (Claude, Codex, Cursor). Expect 3-5 minutes on a warm network.
...
All five must pass. This confirms parity between the container and CI.

### 8. Try the shell helpers (smoke)

`docker/custom_commands.sh` is maintained for **search-text-highlight**: `check`, `fix`, `typecheck`, `test`, `build`, and `codecheck` call the real npm scripts. From `/app` after `install`:

```bash
check                          # eslint + prettier (read-only)
codecheck                      # full CI-style chain (read-only)
```
...
Then **VS Code → Reopen in Container**. The extension uses the same `docker-compose.yaml` and pre-installs Mocha, ESLint, Prettier, GitLens, and EditorConfig extensions per the example.
...
- [ ] `npm run build` produces a `dist/` matching the host's
